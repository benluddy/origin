	...
	g.Context("", func() {
		...
		g.Describe("build with secrets and configMaps", func() {
			...
			g.It("should contain secrets during the source strategy build [apigroup:build.openshift.io][apigroup:image.openshift.io]", func() {
				...
				e2eoutput.TestContainerOutput(context.Background(), oc.KubeFramework(), "test-build-secret-source", pod, 0, []string{
			...
			g.It("should contain secrets during the docker strategy build", func() {
				...
				pod := exutil.GetPodForContainer(corev1.Container{Name: "test", Image: image})
				e2eoutput.TestContainerOutput(context.Background(), oc.KubeFramework(), "test-build-secret-docker", pod, 0, []string{
					"secret1=secret1",
