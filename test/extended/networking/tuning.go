...
var _ = g.Describe("[sig-network][Feature:tuning]", func() {
	...
	g.DescribeTable("pod should not start for sysctls not on whitelist [apigroup:k8s.cni.cncf.io]", func(sysctl, value string) {
		...
		pod := e2epod.NewPodClient(f).Create(context.Background(), podDefinition)
		err = e2epod.WaitForPodCondition(context.Background(), f.ClientSet, namespace, pod.Name, "Failed", 30*time.Second, func(pod *kapiv1.Pod) (bool, error) {
	...
	g.It("pod sysctls should not affect node [apigroup:k8s.cni.cncf.io]", func() {
		...
		nodePod := e2epod.CreateExecPodOrFail(context.Background(), f.ClientSet, f.Namespace.Name, "nodeaccess-pod-", func(pod *kapiv1.Pod) {
	...
	g.It("pod sysctl should not affect existing pods [apigroup:k8s.cni.cncf.io]", func() {
		...
		previousPod := e2epod.CreateExecPodOrFail(context.Background(), f.ClientSet, f.Namespace.Name, "previous-pod-", func(pod *kapiv1.Pod) {
	...
	g.It("pod sysctl should not affect newly created pods [apigroup:k8s.cni.cncf.io]", func() {
		...
		previousPod := e2epod.CreateExecPodOrFail(context.Background(), f.ClientSet, f.Namespace.Name, "sysctl-pod-", func(pod *kapiv1.Pod) {
			pod.ObjectMeta.Annotations = map[string]string{"k8s.v1.cni.cncf.io/networks": fmt.Sprintf("%s/%s", namespace, baseNAD)}
		})
		testNodeName := getPodNodeName(f.ClientSet, previousPod.Namespace, previousPod.Name)
		...
		nextPod := e2epod.CreateExecPodOrFail(context.Background(), f.ClientSet, f.Namespace.Name, "sysctl-pod-", func(pod *kapiv1.Pod) {
			pod.ObjectMeta.Annotations = map[string]string{"k8s.v1.cni.cncf.io/networks": fmt.Sprintf("%s/%s", namespace, baseNAD)}
			pod.Spec.NodeName = testNodeName
		})
