...
func createProberPod(oc *exutil.CLI, proberPodNamespace, proberPodName string) *v1.Pod {
	...
	return frameworkpod.CreateExecPodOrFail(context.Background(), clientset, proberPodNamespace, proberPodName, func(pod *corev1.Pod) {})
}
...
func probeForClientIPs(oc *exutil.CLI, proberPodNamespace, proberPodName, url, targetIP string, targetPort, iterations int) (map[string]struct{}, error) {
	...
	proberPod := frameworkpod.CreateExecPodOrFail(context.Background(), clientset, proberPodNamespace, probePodName, func(pod *corev1.Pod) {
